...
Set_config_user(){
	...
	read -e -p "(默认: potter):" ssr_user
	[[ -z "${ssr_user}" ]] && ssr_user="potter"
...
Set_config_port(){
	...
	read -e -p "(默认: 4545):" ssr_port
	[[ -z "$ssr_port" ]] && ssr_port="4545"
...
Set_config_password(){
	echo "请输入要设置的用户 密码"
	read -e -p "(默认: 123456789):" ssr_password
	[[ -z "${ssr_password}" ]] && ssr_password="123456789"
...
Set_config_obfs(){
 ...
 ${Tip} 注意：如果协议是origin，那么混淆也必须是plain；如果协议不是origin，那么混淆可以是任意的。有的地区需要把混淆设置成plain才好用。因为混淆不总是有效果，要看各地区的策略，有时候不混淆（plain）或者（origin和plain一起使用），让其看起来像随机数据更好。（特别注意：tls 1.2_ticket_auth容易受到干扰！请选择除tls开头以外的其它混淆！！！）&& echo
...
Download_SSR(){
	cd "/usr/local"
	wget -N --no-check-certificate "https://github.com/ShadowsocksR-Live/shadowsocksr/archive/manyuser.zip"
	#git config --global http.sslVerify false
	#env GIT_SSL_NO_VERIFY=true git clone -b manyuser https://github.com/ShadowsocksR-Live/shadowsocksr.git
...
Service_SSR(){
	if [[ ${release} = "centos" ]]; then
		if ! wget --no-check-certificate https://raw.githubusercontent.com/Potterli20/linux-script/master/vpn/service/ssr_centos -O /etc/init.d/ssr; then
		...
		if ! wget --no-check-certificate https://raw.githubusercontent.com/Potterli20/linux-script/master/vpn/service/ssr_debian -O /etc/init.d/ssr; then
			echo -e "${Error} ShadowsocksR服务 管理脚本下载失败 !" && exit 1
		fi
		chmod +x /etc/init.d/ssr
...
# 封禁 BT PT SPAM
BanBTPTSPAM(){
	wget -N --no-check-certificate https://raw.githubusercontent.com/Potterli20/linux-script/master/vpn/service/ban_iptables.sh && chmod +x ban_iptables.sh && bash ban_iptables.sh banall
...
Update_Shell(){
	...
	cd "${file}"
	wget -N --no-check-certificate "https://raw.githubusercontent.com/Potterli20/linux-script/master/vpn/ssr.sh" && chmod +x ssr.sh
