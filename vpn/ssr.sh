...
Set_config_password(){
	...
	read -e -p "(默认: 12345678):" ssr_password
	[[ -z "${ssr_password}" ]] && ssr_password="12345678"
...
Download_SSR(){
	cd "/usr/local/"
	wget -N --no-check-certificate "https://github.com/mzcls/shadowsocksr/archive/master.zip"
...
Update_Shell(){
	sh_new_ver=$(wget --no-check-certificate -qO- -t1 -T3 "https://raw.githubusercontent.com/otterli20/linux-script/master/vpn/ssr.sh"|grep 'sh_ver="'|awk -F "=" '{print $NF}'|sed 's/\"//g'|head -1) && sh_new_type="github"
